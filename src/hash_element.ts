...
export class HashElement extends FASTElement {
  ...
  constructor(file: File) {
    ...
    this.worker.onmessage = (event: MessageEvent<HashWorkerResponse>) => {
      const { type, progress, hash } = event.data;

      if (type === 'hash-progress' && progress !== undefined) {
        this.updateProgress(progress);
      } else if (type === 'hash-complete' && hash !== undefined) {
        this.completeHash(hash);
      }
    };
  ...
  updateProgress(progress: number) {
    this.remaining = this.total * (1 - progress);
    this.elapsed = new Date().getTime() - this.#started.getTime();
  }

  completeHash(hash: string) {
    this.hash = hash;
    this.remaining = 0;
    this.elapsed = new Date().getTime() - this.#started.getTime();
  }
